            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">@lang('admin.features')</h4>
                    </div>
                    <form method="POST" action="{{route('features.update' , $feature->id)}}" id="aboutUs" enctype="multipart/form-data" data-parsley-validate>
                        @method('put')
                        @csrf
                        <div class="card-body">
                            <div class="row">
                                @foreach(config('translatable.locales') as $lang)
                                    <div class="col-xl-6 col-md-6 col-12 mb-1">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="{{$lang}}[title]" value="{{$feature->translate($lang)->title}}" required parsley-trigger="change" placeholder="@lang('admin.title')" />
                                        </div>

                                        <div class="form-group">
                                            <label>@lang('admin.'.$lang.'.description')</label>
                                            <textarea name="{{$lang}}[description]" rows="5" class="form-control" >{{ $feature->translate($lang)->description }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <div class="col-lg-6 col-md-12">
                                    <div class="form-group">
                                        @if($feature->websiteImages()->first())
                                            <img src="{{$feature->websiteImages()->first()->image_path}}" width="200" height="100" class="image-show img-thumbnail img-rounded" style="margin-right: 130px;"/>
                                        @else
    ...
    <script>
        $(function () {
            $('body').on('submit','#aboutUs',function (e) {
                ...
                $.ajax({
                    url: '{{ route('features.update', $feature->id)}}',
                    method: "post",
