            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">@lang('admin.workplan')</h4>
                    </div>
                    ...
                    <form method="POST" id="aboutUs" enctype="multipart/form-data" data-parsley-validate>
                        ...
                        <div class="card-body">
                            <div class="row">
                                @foreach(config('translatable.locales') as $lang)
                                    <div class="col-xl-6 col-md-6 col-12 mb-1">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="{{$lang}}[title]" value="{{$workplan->translate($lang)->title}}" required parsley-trigger="change" placeholder="@lang('admin.title')" />
                                        </div>

                                        <div class="form-group">
                                            <label>@lang('admin.'.$lang.'.description')</label>
                                            <textarea name="{{$lang}}[description]" rows="5" class="form-control" >{{ $workplan->translate($lang)->description }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                <div class="col-lg-6 col-md-12">
                                    <div class="form-group">
                                        @if($workplan->websiteImages()->first())
                                            <img src="{{$workplan->websiteImages()->first()->image_path}}" width="200" height="100" class="image-show img-thumbnail img-rounded" style="margin-right: 130px;"/>
                                        @else
        ...
        <script>
            $(function () {
                $('body').on('submit','#aboutUs',function (e) {
                    ...
                    $.ajax({
                        url: '{{route('workplan.update' , $workplan->id)}}',
                        method: "post",
